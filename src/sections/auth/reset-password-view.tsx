...
export function ResetPasswordView() {
  ...
  // reset function handler/service

  const handleResetForm = (e: React.FormEvent<HTMLInputElement>) => {
    e.preventDefault();
    setClicked(true);
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      (async () => {
        ...
        try {
          ...
          const response: AxiosResponse = await client.patch('resetAdminPassword/', requestData, config);
  ...
  const validateForm = (data: ResetFormData): FormErrors => {
      ...
      {success && (
        ...
        >
          <Alert onClose={handleClose} severity="success" variant="filled" sx={{ width: '100%' }}>
            Password Successfully Reset, Routing to Sign In Page
          </Alert>
        </Snackbar>
      )}
